...
public class Main {
    //Описываем объектную модель, 3 таблицы, хотя бы 2 из них связаны отношением многие к одному
    //После запуска приложения необходимо просканировать структуру классов и найти сущности
    //Создать структуру БД в реляционной СУБД (сгенерировать по структуре классов SQL команды CREATE TABLE ...)
    //Проверить соответствие Объектной модели и реляционной
    //Для каждого класса определить наличие таблицы
    //Для каждого члена класса определить наличие поля в таблице (типы не контролируем)
    //Реализовать интерфейс EntityManager
    //Протестировать
